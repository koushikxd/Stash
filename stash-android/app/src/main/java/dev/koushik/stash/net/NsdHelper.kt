...
class NsdHelper(context: Context) {
    ...
    private val workerThread = HandlerThread("stash-nsd").also { it.start() }
    private val handler = Handler(workerThread.looper)

    /** Blocks the calling thread until first successful resolve or timeout. */
    fun findMac(timeoutMs: Long = 3000, expectedServiceName: String? = null): Mac? {
        val pending = ArrayDeque<NsdServiceInfo>()
        val pendingLock = Any()
        val resolving = AtomicBoolean(false)
        val done = AtomicBoolean(false)
        val resultQueue = LinkedBlockingQueue<Mac>(1)
        ...
        val discoveryListener = object : NsdManager.DiscoveryListener {
            ...
            override fun onServiceFound(info: NsdServiceInfo) {
                ...
                } else if (!name.startsWith("stash-")) return
    ...
    companion object {
        ...
        private const val SERVICE_TYPE = "_stash._tcp."
    }
}
